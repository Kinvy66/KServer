...
Logger::Logger(const std::string &name)
    ...
    // m_formatter.reset(new LoggerFormatter("%d{%Y-%m-%d %H:%M:%S}%T%t%T%N%t%T%F%T[%p]%T[%c]%T%f:%l%T%m%n"));
    m_formatter.reset(new LoggerFormatter("%d{%Y-%m-%d %H:%M:%S}%T%t%T%N%T%F%T[%p]%T[%c]%T%f:%l%T%m%n"));
}
...
LoggerFormatter::ptr LogAppender::getFormatter() {
    MutexType::Lock lock(m_mutex);
    return m_formatter;
}
...
LoggerManager::LoggerManager() {
    m_root.reset(new Logger);   // 调用 Logger 的默认构造，生成一个默认的logger
    m_root->addAppender(LogAppender::ptr(new StdoutLogAppender));
    m_loggers[m_root->m_name] = m_root;  // 将默认的logger加入加入到map中，统一管理
    init();
}
...
/**
 * @brief 类型转换模板类偏特化 String --> LogDefine
 */
template<>
            ...
            if(n["appenders"].IsDefined()) {
                for (size_t x = 0; x < n["appenders"].size(); ++x) {
                    ...
                    if (type == "FileLogAppender") {
                        lad.type = 1;
                        if (!a["file"].IsDefined()) {
                            std::cout << "log config error :fileappender file is invalid, " << a
                                      << std::endl;
                            continue;;
                        }
...
/**
 * @brief 类型转换模板类片特化 LogDefine --> String
 */
template<>
        ...
        g_log_defines->addListener([](const std::set<LogDefine>& old_value,
            ...
            for (auto& i : old_value) {
                ...
                if (it == new_value.end()) {
                    ...
                    logger->setLevel((LogLevel::Level)100);    // 设定一个很高的(不存在）level
                    logger->clearAppenders();
                }
            }
        });
