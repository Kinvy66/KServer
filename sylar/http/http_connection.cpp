...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    do {
        ...
        len += offset;
        data[len] = '\0';
        size_t nparser = parser->execute(data, len + offset, false);
        if (parser->hasError()) {
            return nullptr;
        }
        offset = len - nparser;
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if (rt < 0) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SOCKET_ERROR
                , nullptr, "send request error errno=" + std::to_string(errno)
                + " errstr=" + std::string(strerror(errno)));
    }
...
void HttpConnectionPool::ReleasePtr(sylar::http::HttpConnection* ptr
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
    ...
    if (!conn) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::POOL_GET_CONNECTION
                , nullptr, "poll host: " + m_host + " port:" + std::to_string(m_port));
    }
